...
tempDf <- read.table('../data/SCZ_MasterInteractorTable_full.txt',
	header=T,sep='\t',stringsAsFactors=F)
tempDf <- subset(tempDf,ListName %in% c('CACNA1C_combined','CUL3_wk7','HCN1_combined',
	'RIMS1_combined','SYNGAP1_combined','TCF4_wk1'))
tempDf$Bait <- sapply(strsplit(tempDf$ListName,'_'),'[',1)
tempDf$Dataset <- tempDf$Bait
tempDf$Gene <- tempDf$GeneName
...
baits <- c('CACNA1C','CUL3','HCN1','RIMS1','SYNGAP1','TCF4')
...
pdf('../output/SCZ_CoExp_ViolinPlots.pdf',height=2.5,width=7.5)
...
pdf('../output/SCZ_CoExp-Maynard2021_ViolinPlot.pdf',height=2,width=2.5)
