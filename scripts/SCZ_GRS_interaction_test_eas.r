# Ruize Liu
# GRS analysis 
# For EAS, fit the linear model GRS ~ Int_Non + Case_Control + Int_Non * Case_Control + PCs and test interaction term: Int_Non * Case_Control.
...
pca_dat = read.table('~/ruizeliu/PCA/merge13_pgc_scz_asia.menv.mds_cov', header = T)
...
for (i in c(1:k)){
  ...
  prs = (prs-m) / std
  prs_tab = data.frame(phe, type, prs, C1, C2, C3, C4, C5, C6, C8, C9, C15, C19)
  
  fit = lm(prs ~ phe + type + phe * type + C1 + C2 + C3 + C4 + C5 + C6 + C8 + C9 + C15 + C19, data = prs_tab)
  s = summary(fit)
